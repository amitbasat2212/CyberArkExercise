...
people.forEach(p=>IncreaseSalary(p));
...
let LowerCaseMessage = messagesFromDad.map(me=>me.toLocaleLowerCase());
...
const findCommentByID=(PostId,CommenId)=>{
    return posts.find(p=>p.id==PostId).comments.find(c=>c.id==CommenId)
    
}

console.log(findCommentByID(2, 1))
...
function SomeAndEvery(){
     ...
     console.log(movies.every(m=>m.year>2000)?"Futuristic stuff":"Yawn");
...
let users =[
  ...
  let UserEmailCompanyName = users.map(u=>{return {email:u.email,companyName:u.company.name}})
  console.log(UserEmailCompanyName);
  
  
  //excrice 2 
  let UserZipCodeStart5 = users.filter(u=>u.address.zipcode[0]=="5");
  console.log(UserZipCodeStart5);


  //excrice 3 
  let UserIdZipCode = users.filter(u=>u.address.zipcode[0]=="5").map(u=>u.id);
  console.log(UserIdZipCode);

  //excrice 4 
  let NamesUsers =users.filter(u=>u.name[0].toLocaleLowerCase()=="c").map(m=>m.name);
  console.log(NamesUsers);


  //excrice 5 
  let AllLiveInSouth = users.every(u=>u.address.city=='South Christy');
  console.log(AllLiveInSouth);

  //excrice 6 
  let Suite = users.find(u=>u.address.suite=="Apt. 950").company.name;
  console.log(Suite);


  //excrice 7 
  const NameCompantCity =(user)=>{
    console.log(`${user.name} lives in ${user.address.city} and owns the company ${user.company.name}`)

  }

  users.forEach(u=>NameCompantCity(u));
